...
app.post("/api/scrape-ecom-cheerio", async (req, res) => {
  ...
  if (site !== "All" && site === "Amazon") {
    ...
    console.log("Scraping data from Walmart");
    let products = [];

    try {
      // Save to MongoDB
      products = await scrapeWalmart(url);
      for (const product of products) {
        try {
          let productObj = {
            ...
            reviews: product.rating?.number_of_reviews,
    ...
    console.log("Scraping data from eBay");
    let products = [];

    try {
      // Save to MongoDB
      products = await scrapeBay(url);
      for (const product of products) {
        try {
          let productObj = {
            availability: product.availability,
...
const walmartConfig = {
  siteName: "Walmart",
  url: "https://www.walmart.com/search/?query=laptop",
  containerSelector: "div.search-result-gridview-item-wrapper", // Updated container for each product
  nameSelector: "a.product-title-link > span", // Product name
  priceSelector: "div.price-main span[aria-hidden='true']", // Price (visible text)
  ratingSelector: "span.stars-reviews-count span", // Rating
  reviewsSelector: "span.stars-reviews-count span", // Number of reviews
  vendorSelector: "div.sold-by-section span", // Vendor (if present)
  paginationParam: "&page=", // For paginated scraping
  render: false, // JS rendering not needed unless dynamically loading
};
      ...
      try {
        ...
        $(siteConfig.containerSelector).each((_, element) => {
          ...
          const originalPrice = $(element)
            .find(siteConfig.originalPriceSelector || "")
            .text()
            .trim();
          const description = $(element)
            .find(siteConfig.descriptionSelector || "")
            .text()
            .trim();
          const discount = $(element)
            .find(siteConfig.discountSelector || "")
          ...
          const vendor = $(element)
            .find(siteConfig.vendorSelector || "")
            .text()
            .trim();
          const rating = $(element)
            .find(siteConfig.ratingSelector || "")
            .text()
            .trim();
          const reviews = $(element)
            .find(siteConfig.reviewsSelector || "")
            .text()
            .trim();
          const availability = $(element)
            .find(siteConfig.availabilitySelector || "")
            .text()
            .trim();
          const badge = $(element)
            .find(siteConfig.badgeSelector || "")
            .text()
            .trim();
          const image = $(element)
            .find(siteConfig.imageSelector || "")
            .attr("src");
        ...
        if (attempt === maxRetries) {
          console.error(
            `Failed to scrape ${url} after ${maxRetries} attempts: ${error.message}`
          );
        } else {
          console.error(
            `Retrying ${url} (attempt ${attempt}): ${error.message}`
          );
  ...
  for (let page = 1; page <= 5; page++) {
    ...
    try {
      const response = await fetch(url + page, {
        method: "GET", // Confirm the HTTP method is correct (usually GET for this type of API)
        headers: {
          "Content-Type": "application/json", // Set headers based on Postman configuration
      ...
      data.forEach((item) => {
        products.push({
          ...
          price: item.item_price.value,
          description: item.condition + item.extra_info + item.shipping_cost + item.free_returns,
          rating: item.seller_rating/20 + item.item_rating%20,
          reviews: item.seller_rating_count,
          vendor: item.seller_name,
          badge: item.condition + item.extra_info + item.shipping_cost + item.free_returns,
          url: item.product_url,
        });
      });
      console.log("received walmart data : ", data.length);
    } catch (error) {
    ...
    try {
      ...
      data.items.forEach((item) => {
        products.push({
          ...
          reviews: item.rating?.number_of_reviews,
          vendor: item.seller,
          badge: item.badges,
          url: item.product_url,
        });
      });
      console.log("received walmart data : ", data.items.length);
    } catch (error) {
      console.error("Error fetching data:", error);
    }
  }
